...
- Numeric-hardening verified backup branch: `backup/realmwright-pr13-numeric-verified-2026-07-25`
...
- `backup/realmwright-pr13-numeric-verified-2026-07-25` preserves the complete branch after strict numeric configuration/counter parsing passed 37 Worker tests and TypeScript validation.
...
- Network/provider failures, invalid or empty output, token-limit truncation, failed quota reservations, and corrupt quota counters must not grant extra model spend.
...
- `15b9b0725af7546e9ba0a1e790cd0509cf9b729d` — replaces raw money/security-path `parseInt` usage with strict bounded configuration parsing and fail-closed persisted-counter parsing. It locks per-visitor demo uses to at most five, bounds global spend and output tokens, keeps the paid device cap at at most three, bounds TTLs and request throttles, and rejects corrupt demo/rate-limit counters instead of silently reopening quotas.
...
- Complete Worker suite currently passes with **37 tests**.
...
- Post-numeric read-only cleanup run: `30121600449` — success.
- A malformed temporary staging-file encoding was detected before the guarded numeric run. CI decoded it to the exact locally replayed patch and verified SHA-256 before application; the temporary files were deleted after the verified commit.
...
### Worker numeric-configuration regression validation
...
- Invalid numeric configuration and corrupt quota counters cannot silently expand the free allowance or owner spend.
...
5. Keep all three backup branches intact and update this file and PR #13 after every verified change.
